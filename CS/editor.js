...
document.write('.cMenuOver { background-color: highlight; color: highlighttext; cursor: Default; font-family:µ∏øÚ; font-size: 10pt; padding: 2 12 2 16; }');
...
\* ---------------------------------------------------------------------- */
//document.write('P {margin-top:2px;margin-bottom:2px;}');
...
function editor_updateToolbar(objname,action) {
  ...
  var IDList = Array('Bold','Italic','Underline','StrikeThrough','SubScript','SuperScript','InsertOrderedList','InsertUnorderedList');
...
function editor_setmode(objname, mode) {
  ...
  if (document.readyState != 'complete') {
    setTimeout(function() { editor_setmode(objname,mode) }, 1000);
    return;
  }
 ...
 // src="' +_editor_url+ 'popups/blank.html"
  ...
  //
    ...
    html += 'P {margin-top:2px;margin-bottom:2px;}</style>\n'
      + '</head>\n'
