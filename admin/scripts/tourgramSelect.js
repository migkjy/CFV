...
SS_ENV.ImgPrefix = 'http://hyun.tourgram.co.kr/admin/images';
...
function SS_insertOption (value, innerHTML, idx)
{
  ...
  if (NewOption.innerHTML != innerHTML)
    NewOption.setAttribute('optionHTML', innerHTML);
...
function SS_changeOption (idx, value, innerHTML)
{
  this.checkOptionsDiv();
  ...
  if (this.options[idx].innerHTML != innerHTML)
    this.options[idx].setAttribute('optionHTML', innerHTML);
  else
    this.options[idx].removeAttribute('optionHTML');
...
function SS_syncSelectedIndex ()
{
  ...
  if (this.options[this.selectedIndex].getAttribute('optionHTML') != null) {
    if (this.TitleTable.cells(0).childNodes(0).innerHTML != this.options[this.selectedIndex].optionHTML)
      this.TitleTable.cells(0).childNodes(0).innerHTML = this.options[this.selectedIndex].optionHTML;
  }
  else {
...
function SS_adjustOptionsDiv ()
{
  ...
  this.OptionsDiv.style.top = (this.bListDown) ? parseInt(this.Table.currentStyle.height) : -parseInt(this.OptionsDiv.currentStyle.height);
...
function SS_syncOptions ()
{
  ...
  for (var i=0; i < this.options.length; i++) {
    ...
    if (this.options[i].getAttribute('optionHTML') != null) {
      if (this.OptionsTable.cells(i).childNodes(0).innerHTML != this.options[i].optionHTML)
        this.OptionsTable.cells(i).childNodes(0).innerHTML = this.options[i].optionHTML;
    }
    else {
...
function SS_createTable ()
{
  this.Table = SS_createElement("\
    <table border=0 cellpadding=0 cellspacing=0 style='table-layout:fixed; cursor:default; position:relative; top:0; left:0;'>\
    <tr><td style='position:relative; top:0; left:0;'></td></tr>\
    </table>"
  );
  if (!isNaN(this.width))
...
function SS_createTitleDiv ()
{
  ...
  this.TitleTable.cells(1).innerHTML = "<img src='"+SS_ENV.ImgPrefix+"/btn_down.gif' border=0 align=absmiddle style='margin:0; padding:0;'>";
  this.TitleTable.cells(2).innerHTML = "<img src='"+SS_ENV.ImgPrefix+"/btn_down_s.gif' border=0 align=absmiddle style='margin:0; padding:0;'>";
  this.TitleTable.cells(3).appendChild(this.select);
  this.TitleWrapper = document.createElement(""
    + "<img src='"+SS_ENV.ImgPrefix+"/img_blank.gif'"
    + "  style='position:absolute; top:0; left:0; z-index:1; margin:0; padding:0; width:100%; height:100%;'"
