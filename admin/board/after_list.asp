...
<body>
    ...
    <div style="padding:10px 10px 50px 0px">
        ...
        <form name="s_frm" method="post" action="after_list.asp" style="display:inline; margin:0px;">
                        ...
                        <table  width="100%" border="0" cellpadding="0" cellspacing="0">
                            <tr>
                               ...
                               <td class="bin_pb" width="*%">
                                   ...
                                   <span style="padding:0 8px 0 0;"><img src="/admin/images/top_list.png" border="0" onClick="location.href='after_list.asp'" style="cursor:pointer;border-radius:2px;"></span>
                                   <span><img src="/admin/images/top_ok.png" border="0" onClick="location.href='after_ins.asp'" style="cursor:pointer;border-radius:2px;"></span>
                               </td>
                            </tr>
                        </table>
                    </td>
        ...
        <form name="form1" method="post" action="after_list.asp" style="display:inline; margin:0px;">
        <input type="hidden" name="check_all" value="0">
            ...
            <div class="table_list">
                <table>
                    ...
                    %>
                    <tr>
                        <td class="bin" colspan="6">등록된 내용이 없습니다</td>
                    </tr>
                    <%
                       Else
                    ...
                    <tr <% if i/2 = Int(i/2) Then %>bgcolor="#F9F9F9"<% Else %>bgcolor="#FFFFFF"<% End If %>  onmouseover="this.style.backgroundColor='#D7ECFF';" onmouseout="this.style.backgroundColor='';">
                        <td class="tob1"><span class="checks" style="padding:0 0 0 8px;"><input type="checkbox" name="checkedcard" value="<%=rs("num")%>" id="<%=rs("num")%>"><label for="<%=rs("num")%>"></label></span></td>
                        <td class="tob2"><%=r_num%></td>
                        <td class="tob4">
                            <a href="after_view.asp?num=<%=num%>&gotopage=<%=gotopage%>&s_cont=<%=s_cont%>&cont=<%=cont%>">
                                <%=title%>
                                <% if repl_cnt > 0  then %><span style="color:#0000FF; padding:0 0 0 10px;"><i class="xi-message xi-x"></i> (<%=repl_cnt%>)</span><% end If %>
                                <% if check_new=0 then %><span style="color:#FF0000; padding:0 0 0 10px;"><i class="xi-new xi-x"></i></span><% end if %>
                            </a>
                        </td>
                        <td class="tob2"><%=user_nm%></td>
                        <td class="tob2"><%=ins_dt%></td>
                        <td class="tob2"><%=hit%></td>
                    </tr>
                    <%
                             Rs.movenext
                             i=i+1
                    ...
                    <table border="0" cellpadding="0" cellspacing="0" width="100%">
                        <tr>
                            ...
                            <td align="center">
                                <div class="paginate1">
                                    <div align=center>
                                        <% Call fn_goPage(gotopage,pagecount,pageblock,"after_list.asp", "&s_cont="&s_cont&"&cont="&cont) %>
                                    </div>
                                </div>
                            </td>
                            <td width="100px"></td>
    ...
    function deleteMailList(){
        ...
        if(res==""){
            ...
            if(confirm("정말로 삭제하시겠습니까?\n삭제하시면 복구가 불가합니다")){
                document.form1.action = "relation_del.asp?asp_file=after_list&gotopage=<%=gotopage%>&s_cont=<%=s_cont%>&cont=<%=cont%>&cartseq="+res;
